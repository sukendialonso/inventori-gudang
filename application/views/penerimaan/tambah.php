...
<body id="page-top">
								...
								<form action="<?= base_url('penerimaan/proses_tambah') ?>" id="form-tambah" method="POST">
									...
									<div class="form-row">
										...
										<div class="form-group col-3">
											<label>NIP</label>
											<input type="text" name="nip" value="<?= $this->session->login['nip'] ?>" readonly class="form-control">
										</div>
										<div class="form-group col-3">
											<label>Nama User</label>
											<input type="text" name="nama" value="<?= $this->session->login['nama'] ?>" readonly class="form-control">
										</div>
										<div class="form-group col-2">
										...
										<div class="col-md-4">
											...
											<div class="form-row">
												<div class="form-group col-10">
													<label for="nama_vendorr">Nama Vendor</label>
													<select name="nama_vendorr" id="nama_vendorr" class="form-control">
														<option value="">Pilih Vendor Barang</option>
														<?php foreach ($all_vendorr as $vendorr): ?>
															<option value="<?= $vendorr->nama_vendorr ?>"><?= $vendorr->nama_vendorr ?></option>
														<?php endforeach ?>
													</select>
												</div>
												<div class="form-group col-2">
												...
												<input type="hidden" name="nama_vendorr" value="">
											</div>
										</div>
										<div class="col-md-8">
											<h5>Data Barang</h5>
	...
	<script>
		$(document).ready(function(){
			...
			$('#nama_vendorr').on('change', function(){
				$(this).prop('disabled', true)
				$('#reset').prop('disabled', false)
				$('input[name="nama_vendorr"]').val($(this).val())
			})

			$(document).on('click', '#reset', function(){
				$('#nama_vendorr').val('')
				$('#nama_vendorr').prop('disabled', false)
				$(this).prop('disabled', true)
				$('input[name="nama_vendorr"]').val('')
			})
